...
class UserNewsletterRepositoryTest {
    ...
    @Test
    @DisplayName("[Reports] 기간 내 읽은 뉴스레터만 조회되는지 확인")
    void findByUserIdAndLastViewedAtBetweenAndIsReadTrue_Success() {
        // given
        User user = User.builder().email("read@test.com").nickname("읽음유저").build();
        em.persist(user);
        ...
        UserNewsletter un1 = UserNewsletter.create(user, n1, null, null, "메모1");
        ReflectionTestUtils.setField(un1, "isRead", true);
        ReflectionTestUtils.setField(un1, "lastViewedAt", LocalDateTime.now());
        em.persist(un1);

        UserNewsletter un2 = UserNewsletter.create(user, n2, null, null, "메모2");
        ReflectionTestUtils.setField(un2, "isRead", false);
        ReflectionTestUtils.setField(un2, "lastViewedAt", LocalDateTime.now());
        em.persist(un2);
        ...
        // when
        List<UserNewsletter> results = userNewsletterRepository.findByUserIdAndLastViewedAtBetweenAndIsReadTrue(
                user.getId(),
                LocalDateTime.now().minusDays(1),
                LocalDateTime.now().plusDays(1));
    ...
    @Test
    @DisplayName("[Reports] 최근 읽은 뉴스레터 정렬 및 '읽음' 필터링 확인")
    void findByUserIdAndIsReadTrueOrderByLastViewedAtDesc_Success() {
        // given
        User user = User.builder().email("sort@test.com").nickname("정렬유저").build();
        em.persist(user);

        // [수정] 뉴스레터를 두 개 생성하여 유니크 제약 조건 회피
        Newsletter n1 = Newsletter.builder().title("뉴스레터1").contentUrl("url1").build();
        Newsletter n2 = Newsletter.builder().title("뉴스레터2").contentUrl("url2").build();
        em.persist(n1);
        em.persist(n2);

        UserNewsletter oldRead = UserNewsletter.create(user, n1, null, null, "옛날");
        ReflectionTestUtils.setField(oldRead, "isRead", true);
        ReflectionTestUtils.setField(oldRead, "lastViewedAt", LocalDateTime.now().minusDays(2));
        em.persist(oldRead);

        UserNewsletter recentRead = UserNewsletter.create(user, n2, null, null, "최근");
        ReflectionTestUtils.setField(recentRead, "isRead", true);
        ReflectionTestUtils.setField(recentRead, "lastViewedAt", LocalDateTime.now());
        em.persist(recentRead);

        em.flush();
        em.clear();

        // when
        List<UserNewsletter> results = userNewsletterRepository.findByUserIdAndIsReadTrueOrderByLastViewedAtDesc(user.getId());

        // then
        assertThat(results).hasSize(2);
        assertThat(results.getFirst().getMemo()).isEqualTo("최근"); // 정렬 확인
    }
}