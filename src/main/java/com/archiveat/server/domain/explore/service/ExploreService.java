...
public class ExploreService {
        ...
        public TopicNewslettersResponse getTopicNewsletters(Long userId, Long topicId, Pageable pageable) {
                ...
                Slice<UserNewsletter> newsletterSlice = userNewsletterRepository.findByUserIdAndTopicId(userId, topicId,
                                pageable);

                // 3. 응답 DTO 변환
                List<TopicNewslettersResponse.NewsletterItemResponse> newsletters = newsletterSlice.getContent()
                                .stream()
        ...
        public InboxResponse getInbox(Long userId) {
                ...
                Map<String, InboxResponse.CategoryDto> categoryMap = categoryRepository.findAll().stream()
                                .collect(Collectors.toMap(Category::getName,
                                                c -> new InboxResponse.CategoryDto(c.getId(), c.getName()),
                                                (existing, replacement) -> existing));

                Map<String, InboxResponse.TopicDto> topicMap = topicRepository.findAll().stream()
                                .collect(Collectors.toMap(Topic::getName,
                                                t -> new InboxResponse.TopicDto(t.getId(), t.getName()),
                                                (existing, replacement) -> existing));

                // 3. 날짜별 그룹화 및 DTO 변환
                Map<String, List<UserNewsletter>> groupedByDate = userNewsletters.stream()
                                .collect(Collectors.groupingBy(
                                                un -> un.getCreatedAt().atZone(java.time.ZoneId.of("UTC"))
                                                                .withZoneSameInstant(APP_ZONE).toLocalDate().toString(),
                                                Collectors.toList()));
        ...
        private InboxResponse.InboxItemDto convertToItemDto(
                ...
                return InboxResponse.InboxItemDto.builder()
                                ...
                                .createdAt(un.getCreatedAt().atZone(java.time.ZoneId.of("UTC"))
                                                .withZoneSameInstant(APP_ZONE)
        ...
        @Transactional
        public ClassificationResponse updateInboxClassification(Long userId, Long userNewsletterId,
                ...
                return ClassificationResponse.builder()
                                ...
                                .classificationConfirmedAt(userNewsletter.getConfirmedAt()
                                                .atZone(ZoneId.of("UTC")) // 1. UTC라고 명시
                                                .withZoneSameInstant(APP_ZONE) // 2. KST로 시간 이동 (+9시간)
                                                .format(DateTimeFormatter.ISO_OFFSET_DATE_TIME))
        ...
        public InboxEditResponse getInboxEditData(Long userId, Long userNewsletterId) {
                ...
                List<InboxEditResponse.TopicDto> topics = topicRepository.findAll().stream()
                                .map(t -> new InboxEditResponse.TopicDto(t.getId(), t.getCategory().getId(),
                                                t.getName()))
                                .collect(Collectors.toList());
